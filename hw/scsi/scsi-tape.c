...
static int32_t scsi_tape_emulate_command(SCSIRequest *req, uint8_t *buf)
{
    ...
    case READ_6:
    {
        uint64_t block = req->cmd.xfer;
        r->iov.iov_len = block;
        int ret = blk_pread(s->qdev.conf.blk, s->position,
                            block, outbuf, 0);
        if (ret < 0) {
            scsi_check_condition(r, SENSE_CODE(READ_ERROR));
            return 0;
        }
        s->position += req->cmd.xfer;
        s->bot = false;
        buflen = block;
        break;
    }
    ...
    case READ_10:
    {
        uint32_t num_blocks = (req->cmd.buf[7] << 8) | req->cmd.buf[8];
        uint32_t transfer_len = num_blocks * 512;
        uint64_t file_offset = 0x8 + s->position; /* skip 8 byte .std header. */
        if (transfer_len == 0) {
            scsi_req_complete(&r->req, GOOD);
            return 0;
        }
        /* ensure buffer is large enough */
        if (transfer_len > r->buflen) {
            qemu_vfree(r->iov.iov_base);
            r->buflen = transfer_len;
            r->iov.iov_base = blk_blockalign(s->qdev.conf.blk, r->buflen);
            outbuf = r->iov.iov_base;
            memset(outbuf, 0, r->buflen);
        }
        /* Override the xfer size since QEMU doesn't parse tape READ_10 */
        req->cmd.xfer = transfer_len;
        req->cmd.mode = SCSI_XFER_FROM_DEV;

        int ret = blk_pread(s->qdev.conf.blk, file_offset,
                            transfer_len, outbuf, 0);
        if (ret < 0) {
            scsi_check_condition(r, SENSE_CODE(READ_ERROR));
            return 0;
        }
        s->position += transfer_len;
        s->bot = false;
        r->iov.iov_len = transfer_len;
        r->buflen = transfer_len;
        return transfer_len;
    }
